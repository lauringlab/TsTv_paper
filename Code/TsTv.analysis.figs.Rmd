...
prep.bloom = function(df,dtset,study,gene){
  df %>% 
    ...
    subset(select = c(Mutation,AA_Sub,Fitness.Mean,Fitness.SD,Dataset,Position,TS_TV,S_NS,Gene,Fitness.CV,Study,N)) #,SNC
}
...
#Radical-Conservative Fitness Differences
...
prep.bloom.corr = function(df,dtset,study,gene){
  df %>% 
    ...
    subset(select = c(Mutation,AA_Sub,Fitness.Mean,Fitness.SD,Dataset,Position,SNC,TS_TV,S_NS,Gene,Fitness.CV,Study,N))
...
4) Table II: Spearman - Bloom vs. Lauring
5) Fig. 3: Fisher & CDF - TsTv - Bloom
6) Fig. 4: Fisher & CDF - C/R
7) Table III: Fisher TsTv OR for C/R
...
sig.pl = function(dat,sig.level,bonf = T,nhypo = NULL){
  if(bonf){
...
table1 = merge(n.both,power) %>% arrange(set,-Total) %>% mutate(set = mapvalues(set,"All","All Fitness Levels")) %>% rename(c(Study = "Dataset"))

```
Rearranged in excel and "All" changed to "All Fitness Levels."
...
table2 = bloomxla_corr %>% subset(Corr != "H1N1" & S_NS != "Both", select = c(Corr, n, r, p)) %>% mutate(Corr = map.xstop(Corr)) %>% rename(c(Corr = "Gene", n = "Number of Shared Mutations", r = "rho")) %>% mutate(rho = rho %>% round(2), p = p %>% signif(digits = 2)) %>% rename(c(p = "P value"))

# #asterisks instead of p values:
# table2 = bloomxla_corr %>% subset(Corr != "H1N1" & S_NS != "Both", select = c(Corr, n, r, p)) %>% mutate(Corr = map.xstop(Corr)) %>% rename(c(Corr = "Gene", n = "Number of Shared Mutations", r = "rho")) %>% mutate(sig = ifelse(p<.01,"*",""), rho = rho %>% round(2)) %>% mutate(rho = paste0(rho,sig)) %>% subset(select = - c(sig,p))
...
#pres
pres.fig3.fish.flu = fig2.fish.fnx(dat = fig3.fish.dat,bl.other = "bl",study = bl.levels[1:3],title = NULL,use.ylims = F,text.off = F,point.size = 3.5,facet.label.off = F,bonf = F)
...
No bonferroni correction for these, since just want to know if each of the classes are significant in each of the genes, not if any of them are significant.
...
ggsave("fig4.tstv.pdf",fig4.tstv,width = 16.9,height = 24,units = "cm")
...
```
I think for fig2 and fig3 will have to make each graph individually and align them so can take out facet titles and make things bigger. And change OR breaks just to 0,1,2, and 5 for HIV ENV



##Supplemental Figures & Tables
1) Supplemental Table I: AUC Viable Mutations-Other Viruses
2) Supplemental Figure 1: Coefficient of Variance-Other Viruses
3) Supplemental Figure 2: AUC Decreasing Thresholds
4) Supplemental Table II: Amino Acid Categories
###1) Supplemental Table I: AUC Viable Mutations-Other Viruses
```{r SUPP.TABLE.I}
supp.table1 = each %>%
